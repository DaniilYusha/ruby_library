...
module Statistics
  def top_readers(orders, qty = 1)
    grouped_readers = orders.group_by(&:reader).each_value { |r_orders| r_orders.uniq!(&:book) }
  ...
  def top_books(orders, qty = 1)
    grouped_books = orders.group_by(&:book).each_value { |b_orders| b_orders.uniq!(&:reader) }
